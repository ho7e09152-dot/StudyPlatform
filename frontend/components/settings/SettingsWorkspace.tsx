...
export function SettingsWorkspace({ section = "general" }: { section?: SettingsSection }) {
              ...
              <button
                ...
                onClick={() => {
                  ...
                  void action.then(() => {
                    ...
                    else window.location.replace(APP_ROUTES.workspaces);
  ...
  function ProfileSettings() {
    ...
    const validTimezone = isValidTimezone(timezone);
    const recordNameError = validateStorageRecordName(recordName);
    useUnsavedChanges(dirty && !saving);
    async function submit(event: FormEvent) { event.preventDefault(); if (mode === "demo") { setError("데모 모드에서는 프로필을 변경할 수 없습니다."); return; } if (recordNameError) { setError(recordNameError); return; } setSaving(true); setError(""); try { const updated = await updateAccountProfile({ displayName, repositoryFileName: recordName, timezone, acceptTerms: false, acceptPrivacy: false, confirmMinimumAge: false }); setUser(updated); await syncMembers(); } catch (e) { setError(getUserFacingError(e, "프로필을 저장하지 못했습니다.")); } finally { setSaving(false); } }
    return <form className="settings-form" onSubmit={submit}><section className="settings-section-block"><h3>기본 정보</h3><div className="settings-form-fields"><label className="is-primary"><span>표시 이름</span><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} minLength={2} maxLength={40} required /><small>Workspace, 일정, 제출과 리뷰에서 표시됩니다.</small></label><label><span>학습 기록 이름</span><div className="settings-file-input"><FileText size={16} /><input value={recordName} onChange={(event) => setRecordName(event.target.value)} required maxLength={80} /><em>.md</em></div><small>새 학습 기록 파일 이름에 사용됩니다. 기존 제출 파일 경로는 유지됩니다.</small>{recordNameError ? <small className="field-error">{recordNameError}</small> : null}</label><label><span>개인 시간대</span><TimezoneSelect value={timezone} onChange={setTimezone} describedBy="profile-timezone-help" /><small id="profile-timezone-help">내 화면에서 날짜와 시간을 표시할 때 사용됩니다. Workspace 마감 기준 시간대와는 별개입니다.</small></label></div></section>{error ? <div className="onboarding-error" role="alert">{error}</div> : null}<footer className="settings-form-actions"><span className="settings-save-state" aria-live="polite">{dirty ? "저장하지 않은 변경사항이 있습니다." : "모든 변경사항이 저장되었습니다."}</span><button className="button button--primary" type="submit" disabled={!dirty || saving || displayName.trim().length < 2 || Boolean(recordNameError) || !validTimezone}>{saving ? "저장 중…" : "프로필 저장"}</button></footer></form>;
  }
