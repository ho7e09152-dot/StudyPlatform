...
- **CONFIRMED** GitHub Account linking additionally binds state to the authenticated internal user id and action `LINK`, uses PKCE S256, and requests no OAuth scope because only public identity is needed. GitHub email and repository data are not requested or stored.
- **CONFIRMED** GitHub ProviderAccount stores external user ID, username, optional display name, avatar URL and profile URL. Its token is encrypted under the same `TokenCipher`; GitHub repository data is not processed in this phase.
...
| CONFIRMED | GitLab repository | `.study-workspace/config.yml` 또는 compatibility layout, `session.yml`, 멤버 Markdown submission files, commit history/author name/message | 학습 일정·제출의 원본과 변경 이력 | Study-ing이 OAuth 사용자 token으로 read/write | Study-ing account/workspace 삭제가 GitLab 파일이나 history를 삭제하지 않음 |
...
| GitHub.com | CONFIRMED when linking capability is configured | Connected Account linking and reauthorization only | GitHub user ID, username, display/profile metadata, encrypted OAuth credential | GitHub.com processing region and the legal classification of the transfer are not determined by code | LEGAL REVIEW REQUIRED; repository processing is not yet supported |
