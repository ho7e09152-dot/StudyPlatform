...
이 문서는 세 팀원이 서로 다른 기능을 구현하더라도 동일한 HTTP 상태, 오류 코드, JSON 구조를 사용하도록 만드는 공통 규칙입니다. API의 기계 판독 가능한 원본은 [`openapi.yaml`](openapi.yaml)입니다.
