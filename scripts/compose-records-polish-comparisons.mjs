...
const beforeRoot = path.resolve("artifacts/records-polish-before");
const afterRoot = path.resolve("artifacts/records-polish-after");
...
async function compose(filename, title, device) {
  const rows = [];
  for (const item of [
    { label: "Weekly", path: `${device}/01-weekly.png` },
    { label: "Monthly", path: `${device}/03-monthly.png` },
  ]) {
    rows.push(`<section><h2>${item.label}</h2><div><figure><figcaption>Before polish</figcaption><img src="${await dataUrl(beforeRoot, item.path)}"></figure><figure><figcaption>After polish</figcaption><img src="${await dataUrl(afterRoot, item.path)}"></figure></div></section>`);
  }
...
await compose("comparison-desktop.png", "Records semantic and visual polish · Desktop", "desktop");
await compose("comparison-mobile.png", "Records semantic and visual polish · Mobile", "mobile");
await browser.close();
